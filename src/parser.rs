        ...
        while !input.is_empty() {
            if input.peek(syn::token::Paren) {
                ...
                while mod_content.peek(Token![|]) {
                    ...
                    // Look ahead to see if it's 'replace'
                    if mod_content.peek(syn::Ident) && mod_content.peek2(token::Paren) {
                        let mod_name: Ident = mod_content.parse()?;
                        if mod_name == "replace" {
                            println!("replace fell here 1");

                            let replace_content;
                            parenthesized!(replace_content in mod_content);
                            let from: LitStr = replace_content.parse()?;
                            replace_content.parse::<Token![,]>()?;
                            let to: LitStr = replace_content.parse()?;
                            modifiers.push(Modifier::Replace(from.value(), to.value()));
                        }
                    } else {
                        // ... handle singular/snakecase as before
                        let mod_name: Ident = mod_content.parse()?;
